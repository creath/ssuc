...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SSUCoinTEST2\r");
            Send(hSocket, "WHO #SSUCoinTEST2\r");
        } else {
            // randomly join #SSUCoin00-#SSUCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #SSUCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SSUCoin%02d\r", channel_number).c_str());
        }
