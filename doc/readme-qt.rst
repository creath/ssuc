SSUCoin-qt: Qt4 GUI for SSUCoin
===============================
...
Alternatively, install Qt Creator and open the `SSUCoin-qt.pro` file.

An executable named `SSUCoin-qt` will be built.
...
A warning for people using the *static binary* version of SSUCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of SSUCoin is linked against libdb5.3.
...
installed causes SSUCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
